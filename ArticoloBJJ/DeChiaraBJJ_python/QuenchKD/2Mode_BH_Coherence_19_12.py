...
plt.plot(spectrum_label,(final_eigenvalues-final_eigenvalues[0])/omega_f,'o',markersize=1,color='blue')
plt.xlim(0,100)
plt.ylim(0)
plt.show()
...
U_range=np.geomspace(pow(10,-3)*K,pow(10,3)*K,100)
...
fig1, ax1 = plt.subplots(figsize=(7, 6)) # Figure for Average Work
fig2, ax2 = plt.subplots(figsize=(7, 6)) # Figure for Variance
...
for idx, sigma in enumerate(signs_list):
    ...
    for U_i in U_range:
        ...
        if fourth_exited[origine] <0:
            fourth_exited = -fourth_exited  
        # Initial superposition
        initial_state = 1/np.sqrt(2) * (second_exited+ sigma * fourth_exited)
        ...
        W_analytic.append(N/4*(U_f-U_i)*K/omega_i*(7 + sigma * np.sqrt(12)))
        ...
        term_x4 = 76 + sigma * 14 * np.sqrt(12)
        term_x2_squared = (7 + sigma * np.sqrt(12))**2
        W_var_analytic.append((N*(U_f-U_i)*K/omega_i)**2/16 * (term_x4 - term_x2_squared))
    ...
    ax1.plot(U_range/K, np.abs(Average_quantum_work/N/K),'o',markersize=2,label=f'Numerical ({labels[idx]})',color=colors_num[idx])
    ax1.plot(U_range/K, np.abs(Average_quantum_work_analytic/K/N),label=f'Analytical ({labels[idx]})',color=colors_ana[idx], linestyle='--')

    #Plot of the work variance
    ax2.plot(U_range/K, Work_variance/(K**2),'o',markersize=2,label=f'Numerical ({labels[idx]})',color=colors_num[idx])
    ax2.plot(U_range/K, Work_variance_analytic/(K**2),label=f'Analytical ({labels[idx]})',color=colors_ana[idx], linestyle='--')

ax1.set_xlabel(r'$U_i/K$')
ax1.set_ylabel(r'$\langle W^{\mathrm{quench}}\rangle/K$')
ax1.set_xscale('log')
ax1.set_yscale('log')
ax1.legend()
#ax1.set_title("Average Quantum Work")

ax2.set_xlabel(r'$U_i/K$')
ax2.set_ylabel(r'$\mathrm{Re}\left[(\Delta W^{\mathrm{quench}})^2\right]/K^2$')
ax2.set_xscale('log')
ax2.set_yscale('log')
ax2.legend()
#ax2.set_title("Work Variance")
