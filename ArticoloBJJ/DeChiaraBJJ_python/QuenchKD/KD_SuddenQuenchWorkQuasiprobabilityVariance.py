...
def E_th_func(k, omega):
    """Autovalori oscillatore armonico"""
    return omega * (k + 0.5)
...
# =============================================================================
#  CALCOLO DEI MOMENTI (MEDIA E VARIANZA)
# =============================================================================

# --- 1. MEDIA <W> ---

# A) Numerica (dalla distribuzione num)
W_avg_num = 0
for n in [0, 2]:
    for m in range(N + 1):
...
# B) Teorica (Formula analitica Chap 3)
alpha_sq = np.abs(c_coeffs[0])**2
beta_sq = np.abs(c_coeffs[2])**2
alpha_beta_real = np.real(c_coeffs[0] * np.conj(c_coeffs[2]))
j_idx = 0; k_idx = 2

# Prefattore comune per il primo momento
prefactor_1 = (omega_f**2 - omega_i**2) / (2 * omega_i)

# Termini
term_tpm_1 = alpha_sq * (j_idx + 0.5) + beta_sq * (k_idx + 0.5)
term_coh_1 = alpha_beta_real * np.sqrt((j_idx + 1) * (j_idx + 2)) 

# Medie analitiche
W_avg_TPM_analytic = prefactor_1 * term_tpm_1          # Media se usassimo schema TPM
W_avg_theory_analytic = prefactor_1 * (term_tpm_1 + term_coh_1) # Media KDQ (Full)
...
for n in [0, 2]:
    for m in range(N + 1):
        work_val = E_f[m] - E_i[n]
        W_sq_avg_num += q_num_matrix[m, n].real * (work_val**2)
Var_num = W_sq_avg_num - W_avg_num**2

# B) Teorica (Formule Analitiche Chap 3, Eq 3.93)
# Re<W^2> = <W^2>_TPM + <W^2>_coh

freq_term_sq = (omega_f**2 - omega_i**2)**2 / (omega_i**2)

# Secondo Momento TPM
W2_TPM_analytic = (1.0/16.0) * freq_term_sq * (3 * alpha_sq * (1 + 2*j_idx + 2*j_idx**2) + 
                                               3 * beta_sq  * (1 + 2*k_idx + 2*k_idx**2))

# Secondo Momento Coerenza
term_coh_factor = (4*j_idx + 6) * np.sqrt((j_idx + 1)*(j_idx + 2))
W2_coh_analytic = (1.0/8.0) * freq_term_sq * alpha_beta_real * term_coh_factor

W2_total_analytic = W2_TPM_analytic + W2_coh_analytic

# CALCOLO VARIANZE
# Varianza KDQ (Full) = Re<W^2>_tot - (<W>_tot)^2
Var_theory_analytic = W2_total_analytic - W_avg_theory_analytic**2

# Varianza TPM = <W^2>_TPM - (<W>_TPM)^2
Var_TPM_analytic = W2_TPM_analytic - W_avg_TPM_analytic**2
...
# Barre Numeriche
ax.bar(data_0['W_num'], data_0['q_num'], width=0.7, color='blue', alpha=0.5, 
       edgecolor='blue', linewidth=0.5, label=r'Numerical $q_{m0}$')
ax.bar(data_2['W_num'], data_2['q_num'], width=0.7, color='green', alpha=0.5, 
...
# Inset 1
ax_ins1.bar(data_0['W_num'][:n_points], data_0['q_num'][:n_points], width=1.0, 
...
# Inset 2
ax_ins2.bar(data_2['W_num'][:n_points], data_2['q_num'][:n_points], width=2.0, 
...
text_str = (f"$\\langle W \\rangle$="
            f"{W_avg_theory_analytic:.2f}"
            )
ax.text(center_x-0.1, text_y-0.05, text_str, transform=ax.transAxes,
        fontsize=20, ha='center', va='top',
        bbox=dict(boxstyle="round,pad=0.4", fc="white", ec="gray", alpha=0.9))

text_str = (f"$Re[(\Delta W)^2]$="
            f"{Var_theory_analytic:.2f}"
            )
ax.text(center_x+0.15, text_y-0.05, text_str, transform=ax.transAxes,
        fontsize=20, ha='center', va='top',
        bbox=dict(boxstyle="round,pad=0.4", fc="white", ec="gray", alpha=0.9))
...
ax.text(center_x, text_y - 0.2, text_str_N, transform=ax.transAxes,
        fontsize=20, ha='center', va='top',
...
# =============================================================================
print("-" * 60)
print(f"ANALISI LAVORO E VARIANZA (Sudden Quench U_i={U_i/K}K -> U_f={U_f/K}K)")
print("-" * 60)
print(f"Nonpositivity Functional N:        {nonpositivity_val:.6f}")
print("-" * 60)
print("PRIMO MOMENTO <W>")
print(f"  Numerico (da spettro BJJ):       {W_avg_num:.6f}")
print(f"  Teorico (Full KDQ):              {W_avg_theory_analytic:.6f}")
print("-" * 60)
print("CONFRONTO VARIANZE Re{<(DW)^2>}")
print(f"  Numerico (BJJ):                  {Var_num:.6f}")
print(f"  Teorico Full (KDQ - Eq 93):      {Var_theory_analytic:.6f}")
print(f"  Teorico TPM (Solo popolazioni):  {Var_TPM_analytic:.6f}")
print("-" * 60)