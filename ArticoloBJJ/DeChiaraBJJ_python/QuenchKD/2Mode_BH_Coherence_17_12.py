...
for idx, sigma in enumerate(signs_list):
    ...
    for U_i in U_range:
        ...
        initial_state = 1/np.sqrt(2) * (ground_state + sigma * second_exited)
        ...
        W_analytic.append(N/4*(U_f-U_i)*K/omega_i*(3 + sigma * np.sqrt(2)))
        ...
        term_x4 = 21 + sigma * 6 * np.sqrt(2)
        term_x2_squared = (3 + sigma * np.sqrt(2))**2
